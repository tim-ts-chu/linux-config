...
if !exists('g:git_no_map_default') || !g:git_no_map_default
    ...
    nnoremap <F7>d :GitDiff<CR>
    nnoremap <F7>D :GitDiff --cached<CR>
    nnoremap <F7>s :GitStatus<CR>
    nnoremap <F7>l :GitLog<CR>
    nnoremap <F7>a :GitBlame<CR>
endif
...
function! GitBlame(...)
    let l:git_blame_width = 30
    exec 'lcd '.expand('%:h:p')
    let git_output = s:SystemGit('blame -- ' . expand('%:t'))
    exec 'lcd -'
    if !strlen(git_output)
        echo "No output from git command"
        return
    ...
    nmap <buffer> gd	:let @z=@"<CR>0"xyw:let @"=@z<CR>:tabe<CR>:0r !git show <C-r>x<CR>:setlocal buftype=nofile readonly modifiable<CR>gg:set ft=diff<CR><CR>
